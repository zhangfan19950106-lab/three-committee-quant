...
4. Read `MEMORY.md` — system rules (L0/L1 only)
5. Read `memory_map.md` — decide what else to load based on task
6. **Conditional loads (NOT automatic):**
   - **Trading agent work** → also load `memory/position.md` + `memory/tools.md` + `memory/issues.md`
   - **Coding/debugging** → also load `memory/tools.md` + `memory/issues.md`
   - **Looking for recent context** → read `memory/YYYY-MM-DD.md` (today or the relevant date)

Don't load daily logs or LCM summaries unless needed. System manages LCM automatically.
...
See `memory_map.md` for the full memory index. Key principle:
- **Always load**: SOUL.md, IDENTITY.md, USER.md, MEMORY.md (L0+L1 rules)
- **Task-based load**: position.md (trading), tools.md (coding), issues.md (debugging)
- **Never auto-load**: daily logs, LCM summaries (managed by system)
